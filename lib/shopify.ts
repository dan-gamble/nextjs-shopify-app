...
const context = {
  ...
  API_VERSION: ApiVersion.April21, // all supported versions are available, as well as "unstable"
                                   // and "unversioned"
...
type UpdateableContextParams = {
  HOST_NAME: string
}

export function updateShopifyContext (overrides: UpdateableContextParams) {
  Shopify.Context.initialize({ ...context, ...overrides })
}

export default Shopify

export function ShopifyAuth (config: { afterAuth: Function } = { afterAuth: () => {} }) {
  return (req, res) => {
